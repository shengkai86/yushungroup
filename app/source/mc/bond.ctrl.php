...
if ($do == 'card') {
	...
	if(!empty($setting['fields'])) {
		...
		if(strlen($member_info['email']) == 39 && strexists($member_info['email'], '@yushunbox.com')) {
...
if($do == 'email') {
	...
	if ($_W['member']['email'] == md5($_W['openid']).'@yushunbox.com') {
			...
			if ($reregister) {
				...
				if (!empty($_GPC['email'])) {
					...
					if (($username_type == 'email' || $username_type == 'random') && preg_match(REGULAR_EMAIL, $username)) {
						...
						//当用户已填写重要资料,把用户@yushunbox.com格式的邮箱重置为空,防止用户登陆反复提示重置资料
