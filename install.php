...
if($action == 'env') {
	...
	if(version_compare(PHP_VERSION, '5.3.0') == -1) {
		...
		$ret['php']['version']['remark'] = 'PHP版本必须为 5.3.0 以上. <a href="http://bbs.yushunbox.com/">详情</a>';
	}
	...
	if($ret['php']['pdo']['ok']) {
		...
		if(!$ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['remark'] = '您的PHP环境虽然不支持 mysql_connect, 但已经支持了PDO, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://bbs.yushunbox.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58073">详情</a>';
		}
	} else {
		if($ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['pdo']['class'] = 'warning';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 但支持 mysql_connect, 这样系统虽然可以运行, 但还是建议你开启PDO以提升程序性能和系统稳定性. <a target="_blank" href="http://bbs.yushunbox.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
		} else {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['pdo']['class'] = 'danger';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 也不支持 mysql_connect, 系统无法正常运行. <a target="_blank" href="http://bbs.yushunbox.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
	...
	if($ret['php']['curl']['ok']) {
		...
		if(!$ret['php']['fopen']['ok']) {
			$ret['php']['curl']['remark'] = '您的PHP环境虽然不支持 allow_url_fopen, 但已经支持了cURL, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://bbs.yushunbox.com/wiki/">详情</a>';
		}
	} else {
		if($ret['php']['fopen']['ok']) {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['curl']['class'] = 'warning';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 但支持 allow_url_fopen, 这样系统虽然可以运行, 但还是建议你开启cURL以提升程序性能和系统稳定性. <a target="_blank" href="http://bbs.yushunbox.com/wiki/">详情</a>';
		} else {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['curl']['class'] = 'danger';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 也不支持 allow_url_fopen, 系统无法正常运行. <a target="_blank" href="http://bbs.yushunbox.com/wiki/">详情</a>';
	...
	if($ret['php']['ssl']['ok']) {
		...
		$ret['php']['ssl']['remark'] = '没有启用OpenSSL, 将无法访问公众平台的接口, 系统无法正常运行. <a target="_blank" href="http://bbs.yushunbox.com/wiki/">详情</a>';
	}
	...
	if($ret['php']['gd']['ok']) {
		...
		$ret['php']['gd']['remark'] = '没有启用GD, 将无法正常上传和压缩图片, 系统无法正常运行. <a target="_blank" href="http://bbs.yushunbox.com/wiki/">详情</a>';
	}
	...
	if($ret['php']['dom']['ok']) {
		...
		$ret['php']['dom']['remark'] = '没有启用DOMDocument, 将无法正常安装使用模块, 系统无法正常运行. <a target="_blank" href="http://bbs.yushunbox.com/wiki/">详情</a>';
	}
	...
	if($ret['write']['root']['ok']) {
		...
		$ret['write']['root']['remark'] = '本地目录无法写入, 将无法使用自动更新功能, 系统无法正常运行.  <a href="http://bbs.yushunbox.com/">详情</a>';
	}
 ...
 * 版本号
 *
 * [yushunbox System] Copyright (c) 2013 yushunbox.com
 */
...
function remote_install() {
	...
	$pars['version'] = '0.6';
	$pars['release'] = '';
	$url = 'http://addons.yushunbox.com/gateway.php';
	$ch = curl_init($url);
		...
		<div class="container">
			<div class="header" style="margin:15px auto;">
				<ul class="nav nav-pills pull-right" role="tablist">
					...
					<li role="presentation"><a href="http://www.yushunbox.com">微赞官网</a></li>
					<li role="presentation"><a href="http://bbs.yushunbox.com">访问论坛</a></li>
				</ul>
				<img src="http://bbs.yushunbox.com/static/image/common/logo.png" />
			</div>
			...
			<div class="footer" style="margin:15px auto;">
				<div class="text-center">
					<a href="http://www.yushunbox.com">关于微赞</a> &nbsp; &nbsp; <a href="http://bbs.yushunbox.com">微赞帮助</a> &nbsp; &nbsp; <a href="http://www.yushunbox.com">购买授权</a>
				</div>
				<div class="text-center">
					Powered by <a href="http://www.yushunbox.com"><b>微赞</b></a> v0.6 &copy; 2014 <a href="http://www.yushunbox.com">www.yushunbox.com</a>
				</div>
			</div>
		</div>
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				...
				<p>
					感谢您选择微赞WZ - 微信公众平台自助开源引擎（以下简称yushunbox，yushunbox基于 PHP + MySQL的技术开发，全部源码开放。 <br />
					为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：
				</p>
				<p>
					<strong>一、本授权协议适用且仅适用于微赞系统(yushunbox. 以下简称微赞)任何版本，微赞官方对本授权协议的最终解释权。</strong>
				</p>
				<p>
				...
				<p>
					...
					<ol>
						...
						<li>电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装  yushunbox，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</li>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" value="yushunbox">
					</div>
				</div>
			</div>
		</div>
...
function tpl_install_finish() {
	...
	<div class="alert alert-warning">
		我们强烈建议您立即注册云服务，享受“在线更新”等云服务。<a target="_blank" class="btn btn-success" href="http://bbs.yushunbox.com/">马上去注册</a>
		<br />
	</div>
EOF;
	tpl_frame();
...
function tpl_resources() {
	static $res = array(
		'logo' => 'http://bbs.yushunbox.com/static/image/common/logo.png',
	);
	return $res;
}
