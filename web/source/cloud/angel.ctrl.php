<?php
/**
 * [WDL] Copyright (c) 2013 www.weike.com
 * 折翼天使微赞一键更新程序
 */
load()->func('file');
defined('IN_IA') or exit('Access Denied');
    ...
    $ver = include('angel.php');
    $ver = $ver['ver'];
    $ver = substr($ver,-4);
	$hosturl = urlencode('http://' . $_SERVER['HTTP_HOST']);
	$updatehost = 'http://wzup.weike.com/update.php';
	...
	elseif($_GPC['op'] == 'update'){
		...
		if (strstr($updatenowinfo, 'zip')){			
				...
				if($isgot){				
					...
					message('已将系统更新到最新版本！',create_url('cloud/angel',array('op'=>'display')),'success');
				}
				else{
				...
				message('折翼天使提醒您:请检查授权状态，或者联系折翼天使资源社区授权！',referer(),'error');
		}
	}
